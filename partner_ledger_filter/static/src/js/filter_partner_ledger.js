odoo.define('account_reports.partner_ledger_filter', function(require) {
    'use strict';
    ...
    var accountReportsWidget = AbstractAction.extend(ControlPanelMixin, {
        ...
        init: function(parent, action) {
            ...
            console.log(this.report_options)
            return this._super.apply(this, arguments);
        },
        start: function() {
            var self = this;
        ...
        render_searchview_buttons: function() {
            ...
            if (this.report_options.partner) {
                if (!this.M2MFilters) {
                    ...
                    if (this.odoo_context.model == 'account.partner.ledger'){
        ...
        renderButtons: function() {
            var self = this;
            this.$buttons = $(QWeb.render("accountReports.buttons", {
                buttons: this.buttons
            }));
            // bind actions
        ...
        render_footnotes: function() {
            ...
            _.each($dom_footnotes, function(el) {
                ...
                var footnote = _.filter(self.footnotes, function(footnote) {
        ...
        add_edit_footnote: function(e) {
            ...
            var $content = $(QWeb.render('accountReports.footnote_dialog', {
                text: text,
                line: line_id
            }));
            var save = function() {
                var footnote_text = $('.js_account_reports_footnote_note').val().replace(/[ \t]+/g, ' ');
                if (!footnote_text && existing_footnote.length === 0) {
                    return;
                }
                if (existing_footnote.length !== 0) {
                    if (!footnote_text) {
                    ...
                    return this._rpc({
                            ...
                            args: [{
                                line: line_id,
                                text: footnote_text,
                                manager_id: self.report_manager_id
                            }],
                            context: this.odoo_context,
                        })
                        .then(function(result) {
        ...
        fold_unfold: function(e) {
            var self = this;
            if ($(e.target).hasClass('caret') || $(e.target).parents('.o_account_reports_footnote_sup').length > 0) {
                return;
            }
            e.stopPropagation();
            e.preventDefault();
        ...
        load_more: function(ev) {
            ...
            var options = _.extend({}, this.report_options, {
                lines_offset: offset,
